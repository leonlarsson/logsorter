function parseIDs() {
  ...
  setTimeout(function () { // Setting a timeout of 0 milliseconds so that the above status text gets rendered
    ...
    if ($("#checkBoxUseVerboseLogs").is(":checked")) { // If checked, use verbose logs

      if ($("#textBoxOutput").text() == "") { // Verbose logging. If textarea is empty, add no newlines at the top
        $("#textBoxOutput").append(`Generated on ${dateTime} UTC. Using pattern ${regex} on left text box separated with ${idSeparatorText}:\n`);
    ...
    console.log(`%c[DEBUG] Matched ${matchesFound} lines in ` + ((operationFinish - operationStart) / 1000).toFixed(2) + ` seconds.\n--------------------------------------`, `color: #43b581`); // Once done, console.log the amount of found matches
